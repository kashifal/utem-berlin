...
export const useGenerateReport = (projectP = {}, productP = {}) => {

  project.value = projectP ? projectP : {}
  product.value = productP ? productP : {}
  ...
  for (let step = 1; step <= project.value?.coating?.coatingProduct.features.length; step++) {
    doc.value.text(`-`, contentStartX, yPosition);
    yPosition = addWrappedText({
      /*text: product.value.features[step - 1],*/
      text: project.value?.coating?.coatingProduct.features [step - 1],
      textWidth: textWidth,
      fontSize: "9",
      fontType: "normal",
  ...
  yPosition += headlineDistance;
    ...
    for (let step = 1; step <= project.value?.coating?.coatingProduct.anwendungen.length; step++) {
    doc.value.text(`-`, contentStartX, yPosition);
    yPosition = addWrappedText({
      text: project.value?.coating?.coatingProduct.anwendungen [step - 1],
      textWidth: textWidth,
      fontSize: "9",
      fontType: "normal",
      lineSpacing: lineSpacing,
  ...
  doc.value.save(
    `${project.value?.coating?.coatingProduct?.product} technisches Produktblatt.pdf`
  );
}
...
const generatePdfLayout = () => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.value.text(`${project.value?.coating?.coatingProduct?.product}`,15,30);
